...
to move
  ...
  [                                                    ;ig better if there are specific patches that is either their home or the hospital to be quarantined in (future endeavor)
   if extraversion-level = "High" [set heading (towards one-of neighbors) move-to one-of neighbors]
   if extraversion-level = "Med" [right random 150 left random 150 fd 1]
    if extraversion-level = "Low" [if any? other turtles-on patch-here or not any? turtles-on patch-ahead 1 [right random 150 left random 150 fd 1]]
  ]
end
